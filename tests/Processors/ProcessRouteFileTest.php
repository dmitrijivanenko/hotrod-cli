...
class ProcessRouteFileTest extends TestCase
{
    /**
     * @test
     */
    public function it_should_call_the_create_command()
    {
        ...
        $input->getOption()->shouldBeCalled()->withArguments(['no-routes'])->willReturn(false);
        ...
        $application->find('create:route')->shouldBeCalled()->willReturn($command->reveal());

        $processor = new ProcessRouteFile($appContainer->reveal());
